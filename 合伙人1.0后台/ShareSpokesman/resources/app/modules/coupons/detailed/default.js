    ...
    that.connect(( err , db , conn ) => {
        db.beginTransaction(function(err){
            let sql  = "SELECT a.*,b.start_time AS start_time,b.end_time AS end_time, b.headline, b.types, b.worth, b.content, b.backdrop, b.status AS coupons_status FROM ";
                ...
                sql += "WHERE b.id = a.coupons_id AND a.manager_id = ? AND a.id = ? LIMIT 1 ";
            db.query( sql , [ USER.manager_id , coupons_record_id ] , function(err, row, fields) {
                if( err || !row ){
                    endQuery();
                ...
                db.query( "SELECT "+ field +" FROM ?? WHERE id IN( ? , ? , ? )" , [ 'db_people' , coupons.clerk_id | 0, coupons.used_clerk_id | 0 , coupons.prolocutor_id | 0 ] , function(err, row, fields) {
                    if( !err && row ){
                        for( var index in row ){
                            ...
                            if( row[index].id == coupons.prolocutor_id ){
