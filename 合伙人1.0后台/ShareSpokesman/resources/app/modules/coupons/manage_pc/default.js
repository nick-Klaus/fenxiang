...
exports.ready = (req, res, that) => {
    ...
    that.connect(( err , db , conn ) => {
        ...
        let sql  = "SELECT count(*) AS total FROM `db_coupons` AS a, `db_coupons_access` AS b WHERE ( b.coupons_id = a.id AND b.manager_id = ? AND b.shopowner_id = ? "+ search +" ) LIMIT 1 ";
        db.query( sql , [ body.manager_id , body.shopowner_id ] , function(err, row, fields) {
            ...
            let sql  = "SELECT a.*, b.shopowner_id AS access_shopowner_id, b.id AS access_id FROM `db_coupons` AS a, `db_coupons_access` AS b WHERE ( b.coupons_id = a.id AND b.manager_id = ? AND b.shopowner_id = ? "+ search +" ) LIMIT ?, ? ";
            db.query( sql , [ body.manager_id , body.shopowner_id , result.currentpage , result.pagesize ] , function(err, list, fields) {
                endQuery(); // 结束当前数据库链接
                result.list = list || [];
                return res.successEnd( result );
            });
