...
exports.ready = (req, res, that) => {
    ...
    that.connect((err, db,conn) => {    
        ...
        db.query("select count(id) num  from db_order  where sid =? and status =1 and pay_status=3   "+add_sql_order, [body.shopowner_id] , function(err, row1, fields) {
            if (err) {
                db.release();// 释放资源
                conn.end(); // 结束当前数据库链接
            ...
            var sql = "select a.*,b.nickname ,b.mobile from db_order a  left join db_people b on a.cid = b.id  where sid =? and status =1 and pay_status=3  "+add_sql_order+"  limit  " + _currentpage + "," + _pagesize;
            db.query(sql,[body.shopowner_id] ,  function(err1, row, fields1) {
                if (err) {
                    db.release();// 释放资源
                    conn.end(); // 结束当前数据库链接
