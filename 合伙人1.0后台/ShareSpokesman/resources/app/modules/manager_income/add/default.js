    ...
    that.connect(( err , db , conn ) => {
        db.beginTransaction(function(err){
            ...
            if( body.activity_type == 1 ){
                 _table = "db_single_product";
            }
            if( body.activity_type == 2 ){
                 _table = "db_bargain";
            }
            if( body.activity_type == 3 ){
                var _table = "db_crowd_funding";
            }
            if( body.activity_type == 6 ){
                 _table = "db_leaflet";
            }
            if( body.activity_type == 4 ){
                var _table = "db_fight_groups";
            }
            if( _table == "" ){
                db.rollback();
                db.release();// 释放资源
            ...
            db.query("select count(id) as num from `db_activity_record` where activity_type=? and activity_id=? and user_id=? and operation_type=? and openid=? and addtime=?", [body.activity_type,body.activity_id,body.spokesman_id,body.type,body.openid,_addtime], function (err, row1, fields) {
                ...
                db.query("select forward_time,share_time,see_time from `db_people_day_rule` where  manager_id=?", [body.manager_id], function (err, row2, fields) {
                    ...
                    if( row1[0].num >= this_type ){
                        record_arr.commission_type = body.type;
                        record_arr.commission = "次数用完没有佣金或者活动没有奖励";
                        db.query( "INSERT INTO `db_activity_record` SET ?" , record_arr , function( err, result , fields ){
                            ...
                            db.release(); // 释放资源
                            conn.end(); // 结束当前数据库链接
                            return res.successEnd("次数用完没有佣金或者活动没有奖励" );
                        });
                    }else{
                        // 查寻出当前活动的活动启动金还剩多少
                        db.query("select id,start_up_money_all,start_up_money_residue from ?? where id=?", [_table,body.activity_id], function (err, row, fields) {
                            ...
                            if( _old_money > 0 && _old_money >= _money && _money > 0 ){
                                // 减掉活动奖励后剩余的资金 并修改
                                var new_money =  that.floatSub(parseFloat(_old_money),parseFloat(_money));
                                    ...
                                    db.query( "INSERT INTO `db_manager_income` SET ?" , income_arr , function( err, result , fields ){
                                        ...
                                        db.query( "INSERT INTO `db_activity_record` SET ?" , record_arr , function( err, result , fields ){
                                            ...
                                            db.query("select * from `db_activity_log` where user_id=? and boss_id=?", [body.spokesman_id, body.manager_id], function (err, row1, fields) {
                                                ...
                                                var _share_price_new = that.floatAdd(parseFloat(row1[0].share_price_new),parseFloat(share_price));// 今日分享金

                                                var _share_price_month = that.floatAdd(parseFloat(row1[0].share_price_month),parseFloat(share_price));// 月分享金

                                                var _share_price_all = that.floatAdd(parseFloat(row1[0].share_price_all),parseFloat(share_price));// 总的分享金

                                                var _profit_new = that.floatAdd(parseFloat(row1[0].profit_new),parseFloat(share_price));// 今日收益

                                                var _profit_month = that.floatAdd(parseFloat(row1[0].profit_month),parseFloat(share_price));// 本月收益

                                                var _profit_all = that.floatAdd(parseFloat(row1[0].profit_all),parseFloat(share_price));// 总的收益
                                ...
                                // 活动没有启动资金  或者 操作没有奖励
                                if( _money_all == 0 || _money <=0 ){
                                    record_arr.commission_type = body.type;
                                    record_arr.commission = "本操作没有奖励";
                                    db.query( "INSERT INTO `db_activity_record` SET ?" , record_arr , function( err, result , fields ){
                                        if( err ){
                                            db.rollback();
                                            db.release();// 释放资源
