...
exports.ready = (req, res, that) => {
    ...
    that.connect((err, db,conn) => {   
        ...
        if(!that.isEmpty(body.param_text)){
            ...
            db.query("select * from db_search_log where search_type = 4 and manager_id =? and content =?", [body.manager_id,search_text], function(err, search_log, fields) {
                ...
                if(search_log.length>0){
                    ...
                    db.query("INSERT INTO db_search_log ( manager_id, search_type, search_date, content) VALUES (?,4, unix_timestamp(now()),?);", [body.manager_id,search_text], function(err, row, fields) {
                        // 数据获取失败
    ...
    db.query("select count(a.id) num from (`db_people` a left join `db_shopowner` b on a.shopowner_id = b.id) left join `db_shop` c on b.shop_id = c.id where a.manager_id = ? and a.is_clerk = '1' AND ( a.nickname LIKE ? or a.realname LIKE ? or a.mobile like ? ) ", [body.manager_id,body.param_text,body.param_text,body.param_text] , function(err3, row3, fields3) {
        if (err3) {
            db.release();// 释放资源
            conn.end(); // 结束当前数据库链接
        ...
        var sql =   "select a.*,c.shop_name from (`db_people` a left join `db_shopowner` b on a.shopowner_id = b.id) left join `db_shop` c on b.shop_id = c.id where a.manager_id = ? and a.is_clerk = '1' AND ( a.nickname LIKE ? or a.realname LIKE ? or a.mobile like ? ) limit  " + _currentpage + "," + _pagesize;    
        db.query(sql, [body.manager_id,body.param_text,body.param_text,body.param_text] , function(err, row, fields) {
            if(row.length == 0 ){
