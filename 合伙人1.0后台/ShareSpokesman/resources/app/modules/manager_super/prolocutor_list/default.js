...
exports.ready = (req, res, that) => {
    ...
    that.connect((err, db,conn) => {
       ...
       db.query("SELECT count(1) as num  from db_people   where is_prolocutor='1' and  shopowner_id = ?   order by id desc ",[body.shopowner_id], function(err, row1, fields) {
                // 数据获取失败
                if (err) {
                    db.release();// 释放资源
                    conn.end(); // 结束当前数据库链接
                ...
                var sql = "SELECT *  from db_people   where is_prolocutor='1' and  shopowner_id = ?   order by id desc limit "+ _currentpage + "," + _pagesize;
