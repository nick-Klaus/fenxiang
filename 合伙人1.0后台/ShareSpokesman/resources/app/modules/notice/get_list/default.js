...
exports.ready = (req, res, that) => {
   ...
   // 查询数据
    ...
    that.connect(( err , db , conn ) => {
            ...
            db.query("SELECT count(1) as num from db_notice where (notice_type=1 or  pid =?) and   id  not in (select notice_id from db_noticet_record where pid =?  and  status=999)  and manager_id=?   order by create_time desc  ",[body.pid,body.pid,body.manager_id], function(err, row1, fields) {
                // 数据获取失败
                if (err) {
                    db.release();// 释放资源
                    conn.end(); // 结束当前数据库链接
                ...
                db.query("select * ,0 record_status  from db_notice where (notice_type=1 or  pid =?) and   id  not in (select notice_id from db_noticet_record where pid =?  and  status=999)  and manager_id=?  order by create_time desc    limit "+ _currentpage + "," + _pagesize,[body.pid,body.pid,body.manager_id], function(err, row, fields) {
                    ...
                    if( "object" === typeof notice ){
                        db.query("select * from db_noticet_record where pid=? and notice_id=?  ",[body.pid,notice.id], function(err, row_db_noticet_record, fields) {
