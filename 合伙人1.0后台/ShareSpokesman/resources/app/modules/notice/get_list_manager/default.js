...
exports.ready = (req, res, that) => {
    ...
    that.connect(( err , db , conn ) => {
            ...
            db.query("SELECT count(1) as num  from db_notice_manager where (notice_type=1 or  manager_id =?) and   id  not in (select notice_id from db_noticet_record_manager where manager_id =?  and  status=999)  order by create_time desc  ",[body.manager_id,body.manager_id], function(err, row1, fields) {
                // 数据获取失败
                if (err) {
                    db.release();// 释放资源
                    conn.end(); // 结束当前数据库链接
                ...
                db.query("select * ,0 record_status  from db_notice_manager where (notice_type=1 or  manager_id =?) and   id  not in (select notice_id from db_noticet_record_manager where manager_id =?  and  status=999) order by create_time desc    limit "+ _currentpage + "," + _pagesize,[body.manager_id,body.manager_id], function(err, row, fields) {
