    ...
    that.connect((err, db, conn) => {
        ...
        db.query("select * from `db_activity_log` where user_id=? and boss_id=?", [body.user_id, body.boss_id], function (err, row1, fields) {
            ...
            db.query("UPDATE `db_activity_log` SET ? where id=?", [activity_log, row1[0].id], function (err, result, fields) {
                ...
                db.query("select * from `db_bargain` where id=?", [body.bargain_id], function (err, row2, fields) {
                    ...
                    // 更新数据
                    var bargain_log = {"user_fx": _share, "user_zf": _forward,"user_ck":_see}
                    db.query("UPDATE `db_bargain` SET ? where id=?", [bargain_log, row2[0].id], function (err, result, fields) {
                        var total = result.changedRows; // 受影响的数据条数, ( 已删除的数据条数 )
                        if (err) {
                            db.rollback();
                            db.release();// 释放资源
                        ...
                        db.query("select * from `db_bargain_user` where bargain_id=? and user_id=?", [body.bargain_id, body.user_id], function (err, row3, fields) {
                            ...
                            // 更新数据
                            var bargain_user = {"user_fx": _share, "user_zf": _forward,"user_ck":_see}
                            db.query("UPDATE `db_bargain_user` SET ? where id=?", [bargain_user, row3[0].id], function (err, result, fields) {
                                var total = result.changedRows; // 受影响的数据条数, ( 已删除的数据条数 )
                                if (err) {
                                    db.rollback();
                                    db.release();// 释放资源
                            ...
                            db.query("select * from `db_people_task` where user_id=? and manager_id=? and addtime=?", [body.user_id, body.boss_id, nowdate], function (err, row4, fields) {
                                ...
                                if( row4.length == 0 ){
                                    ...
                                    // 更新数据
                                    var bargain_user = {"share": _share, "forward": _forward,"see":_see_task}
                                    db.query("UPDATE `db_people_task` SET ? where id=?", [bargain_user, row4[0].id], function (err, result, fields) {
                                        var total = result.changedRows; // 受影响的数据条数, ( 已删除的数据条数 )
                                        if (err) {
                                            db.rollback();
                                            db.release();// 释放资源
