    ...
    that.connect((err, db, conn) => {
        ...
        db.query("select * from `db_activity_log` where user_id=? and boss_id=?", [body.user_id, body.boss_id], function (err, row1, fields) {
            ...
            db.query("UPDATE `db_activity_log` SET ? where id=?", [activity_log, row1[0].id], function (err, result, fields) {
                ...
                db.query("select * from `db_fight_groups` where id=?", [body.fight_groups_id], function (err, row2, fields) {
                    // 数据获取失败
                    ...
                    // 更新数据
                    var single_product_log = {"user_fx": _share, "user_zf": _forward,"user_ck":_see}
                    db.query("UPDATE `db_fight_groups` SET ? where id=?", [single_product_log, row2[0].id], function (err, result, fields) {
                        var total = result.changedRows; // 受影响的数据条数, ( 已删除的数据条数 )
                        if (err) {
                            db.release();// 释放资源
                            conn.end(); // 结束当前数据库链接
                        ...
                        db.query("select * from `db_fight_groups_user` where fight_groups_id=? and user_id=?", [body.fight_groups_id, body.user_id], function (err, row_num, fields) {
                            // 数据获取失败
                            if (err) {
                                db.release();// 释放资源
                                conn.end(); // 结束当前数据库链接
                            ...
                            if( row_num.length == 0 ){
                                db.query("select * from `db_fight_groups_frist_user` where fight_groups_id=? and user_id=?", [body.fight_groups_id, body.user_id], function (err, row3, fields) {
                                    // 数据获取失败
                                    if (err) {
                                        db.release();// 释放资源
                                        conn.end(); // 结束当前数据库链接
                                    ...
                                    if (row3.length == 0) {
                                        db.release();// 释放资源
                                        conn.end(); // 结束当前数据库链接
                                        return res.successEnd('没有找到用户参与数据！！');
                                    }
                                    ...
                                    // 更新数据
                                    var single_product_user = {"user_fx": _share, "user_zf": _forward,"user_ck":_see}

                                    db.query("UPDATE `db_fight_groups_frist_user` SET ? where id=?", [single_product_user, row3[0].id], function (err, result, fields) {
                                        var total = result.changedRows; // 受影响的数据条数, ( 已删除的数据条数 )
                                        if (err) {
                                            db.release();// 释放资源
                                            conn.end(); // 结束当前数据库链接
                                        ...
                                        db.query("select * from `db_people_task` where user_id=? and manager_id=? and addtime=?", [body.user_id, body.boss_id, nowdate], function (err, row4, fields) {
                                            ...
                                            if( row4.length == 0 ){
                                                ...
                                                // 更新数据
                                                var single_product_user = {"share": _share, "forward": _forward,"see":_see_task}
                                                db.query("UPDATE `db_people_task` SET ? where id=?", [single_product_user, row4[0].id], function (err, result, fields) {
                                                    var total = result.changedRows; // 受影响的数据条数, ( 已删除的数据条数 )
                                                    if (err) {
                                                        db.release();// 释放资源
                                                        conn.end(); // 结束当前数据库链接
                                ...
                                db.query("select * from `db_fight_groups_user` where fight_groups_id=? and user_id=?", [body.fight_groups_id, body.user_id], function (err, row3, fields) {
                                    // 数据获取失败
                                    if (err) {
                                        db.release();// 释放资源
                                        conn.end(); // 结束当前数据库链接
                                    ...
                                    db.query("UPDATE `db_fight_groups_user` SET ? where id=?", [single_product_user, row3[0].id], function (err, result, fields) {
                                        var total = result.changedRows; // 受影响的数据条数, ( 已删除的数据条数 )
                                        if (err) {
                                            db.release();// 释放资源
                                            conn.end(); // 结束当前数据库链接
