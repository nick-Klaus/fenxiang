    ...
    that.connect((err, db, conn) => {
        ...
        db.query("select * from `db_activity_log` where user_id=? and boss_id=?", [body.user_id, body.boss_id], function (err, row1, fields) {
            ...
            db.query("UPDATE `db_activity_log` SET ? where id=?", [activity_log, row1[0].id], function (err, result, fields) {
                ...
                db.query("select * from `db_leaflet` where id=?", [body.leaflet_id], function (err, row2, fields) {
                    ...
                    // 更新数据
                    var leaflet_log = {"user_fx": _share, "user_zf": _forward,"user_ck":_see}
                    db.query("UPDATE `db_leaflet` SET ? where id=?", [leaflet_log, row2[0].id], function (err, result, fields) {
                        var total = result.changedRows; // 受影响的数据条数, ( 已删除的数据条数 )
                        if (err) {
                            db.release();// 释放资源
                            return res.errorEnd('更新数据失败!!', 200);
                        ...
                        db.query("select * from `db_leaflet_user` where leaflet_id=? and user_id=?", [body.leaflet_id, body.user_id], function (err, row3, fields) {
                            // 数据获取失败
                            if (err) {
                                return res.errorEnd('没有找到可用数据', 200);
                            }
                            ...
                            // 更新数据
                            var leaflet_user = {"user_fx": _share, "user_zf": _forward,"user_ck":_see}
                            db.query("UPDATE `db_leaflet_user` SET ? where id=?", [leaflet_user, row3[0].id], function (err, result, fields) {
                                var total = result.changedRows; // 受影响的数据条数, ( 已删除的数据条数 )
                                if (err) {
                                    db.release();// 释放资源
                                    return res.errorEnd('更新数据失败!!', 200);
                            ...
                            db.query("select * from `db_people_task` where user_id=? and manager_id=? and addtime=?", [body.user_id, body.boss_id, nowdate], function (err, row4, fields) {
                                ...
                                if( row4.length ==0 ){
                                    ...
                                    // 更新数据
                                    var leaflet_user = {"share": _share, "forward": _forward,"see":_see_task}
                                    db.query("UPDATE `db_people_task` SET ? where id=?", [leaflet_user, row4[0].id], function (err, result, fields) {
                                        var total = result.changedRows; // 受影响的数据条数, ( 已删除的数据条数 )
                                        if (err) {
                                            db.release();// 释放资源
                                            return res.errorEnd('更新数据失败!!', 200);
                                        ...
                                        if (!total) {
                                            db.release(); // 释放资源
                                            return res.errorEnd('没有找到可操作的数据4!!', 200);
                                        }
                                        db.release(); // 释放资源
                                        conn.end(); // 结束当前数据库链接
