    ...
    that.connect((err, db, conn) => {
        ...
        db.query("select * from `db_activity_log` where user_id=? and boss_id=?", [body.user_id, body.boss_id], function (err, row1, fields) {
            ...
            db.query("UPDATE `db_activity_log` SET ? where id=?", [activity_log, row1[0].id], function (err, result, fields) {
                ...
                db.query("select * from `db_single_product` where id=?", [body.single_product_id], function (err, row2, fields) {
                    ...
                    db.query("UPDATE `db_single_product` SET ? where id=?", [single_product_log, row2[0].id], function (err, result, fields) {
                        var total = result.changedRows; // 受影响的数据条数, ( 已删除的数据条数 )
                        if (err) {
                            db.rollback();
                            db.release();// 释放资源
                        ...
                        db.query("select * from `db_single_product_user` where single_product_id=? and user_id=?", [body.single_product_id, body.user_id], function (err, row3, fields) {
                            // 数据获取失败
                            if (err) {
                                db.rollback();
                                db.release();// 释放资源
                            ...
                            db.query("UPDATE `db_single_product_user` SET ? where id=?", [single_product_user, row3[0].id], function (err, result, fields) {
                                var total = result.changedRows; // 受影响的数据条数, ( 已删除的数据条数 )
                                if (err) {
                                    db.rollback();
                                    db.release();// 释放资源
