...
exports.ready = (req, res, that) => {
    ...
    that.connect((err, db,conn) => {
        //查询未审核的订单
        if(body.status == 0){
            db.query("select * from `db_order` where status = 0 and sid = ?  order by id desc", user_shopowner.id, function(err, row, fields) {
                // 数据获取失败
                if( err ){
                    db.release();// 释放资源
                    conn.end(); // 结束当前数据库链接
            ...
            db.query("select * from `db_order` where status = 1 and sid = ?  order by id desc", user_shopowner.id, function(err, row, fields) {
                // 数据获取失败
                if( err ){
                    db.release();// 释放资源
                    conn.end(); // 结束当前数据库链接
             ...
             db.query("select * from `db_order` where status = 2 and sid = ?  order by id desc", user_shopowner.id, function(err, row, fields) {
                // 数据获取失败
                if( err ){
                    db.release();// 释放资源
                    conn.end(); // 结束当前数据库链接
