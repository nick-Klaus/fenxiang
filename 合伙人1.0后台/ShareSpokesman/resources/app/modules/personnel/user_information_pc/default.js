...
exports.ready = (req, res, that) => {
    ...
    that.connect((err, db,conn) => { 
        db.query("select openid_jhkj,mobile,nickname,is_manager,is_shopowner,id,manager_id,shopowner_id,clerk_id,prolocutor_id,is_clerk,is_prolocutor,fans_id,openid,token from `db_people` where id = ?", body.user_id , function(err, row, fields) {
        // 数据获取失败
            if (err) {
            ...
            if(row.length){
                ...
                return res.errorEnd('找不到该用户', 300);
            }
