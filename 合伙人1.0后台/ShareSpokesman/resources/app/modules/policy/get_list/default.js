...
exports.ready = (req, res, that) => {
    ...
    that.connect((err, db,conn) => {
        ...
        //记录查询日志
            ...
            db.query("select count(1) num from `db_policy` a where a.pid = ? or a.cid = ?   order by a.id desc", [body.pid,body.pid], function(err, row, fields) {
                ...
                var sql = "select * from `db_policy` a where a.pid = ? or a.cid = ?   order by a.id desc limit  " + _currentpage + "," + _pagesize;
                db.query(sql, [body.pid,body.pid], function(err1, row1, fields1) {
