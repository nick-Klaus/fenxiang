...
exports.ready = (req, res, that) => {
    ...
    db.query("select a.id,a.nickname,a.headimgurl,a.realname,a.mobile,b.company,a.is_shopowner,a.is_manager from `db_people` a left join `db_manager` b on a.manager_id = b.id  where a.token = ?", body.token  , function(err, row, fields) {
    // 数据获取失败
        if (err) {
            db.release();// 释放资源
            conn.end(); // 结束当前数据库链接
