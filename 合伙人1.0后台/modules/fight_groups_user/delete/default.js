...
exports.ready = (req, res, that) => {
    ...
    if(that.isEmpty(body.id)){
        return res.errorEnd('删除条件不存在！！', 300);
    }
    that.connect(( err , db , conn ) => {
        db.query( "DELETE FROM db_fight_groups_user WHERE id = ? and fight_groups_id=?" , [body.id,body.fight_groups_id] , function( err, result , fields ){
