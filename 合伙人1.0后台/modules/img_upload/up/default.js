...
exports.ready = (req, res, that) => {
    ...
    // that.connect(( err , db , conn ) => {
        ...
        var base64Data = base64Arr[0].match(/^data:([a-zA-Z]+)\/([a-zA-Z]+);base64$/);
        ...
        if( base64Data && base64Data.length == 3 ){
            ...
            var img_name =Math.ceil( d.getTime() )+"."+base64Data[2];
            var file = path_img+"/"+img_name;
            var dataBuffer  = new Buffer(base64Arr[1], 'base64');
            fs.writeFile( file, dataBuffer,function(err) {
                if(err){
                    return res.errorEnd("图片上传失败",200);
                }else{
                    return res.successEnd("static/"+str_day+"/"+img_name);
                }
            });
        }else{
            return res.errorEnd("图片格式错误",200);
