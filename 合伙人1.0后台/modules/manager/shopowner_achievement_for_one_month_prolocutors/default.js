...
exports.ready = (req, res, that) => {
    ...
    that.connect((err, db,conn) => {    
        ...
        db.query("select count(id) num  from db_people where is_prolocutor='1' and is_clerk='0' and shopowner_id=?   "+add_sql_people, [body.shopowner_id] , function(err, row1, fields) {
            if (err) {
                db.release();// 释放资源
                conn.end(); // 结束当前数据库链接
            ...
            var sql = "select * from db_people where is_prolocutor='1' and is_clerk='0' and shopowner_id=?   "+add_sql_people+"  limit  " + _currentpage + "," + _pagesize;
