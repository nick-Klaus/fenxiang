...
exports.ready = (req, res, that) => {
    ...
    that.connect((err, db,conn) => {    
        ...
        if(!that.isEmpty(body.param_text)){
            ...
            db.query("select * from db_search_log where search_type = 6 and manager_id =? and content =?", [user.manager_id,search_text], function(err, search_log, fields) {
                ...
                if(search_log.length>0){
                    ...
                    db.query("INSERT INTO db_search_log ( manager_id, search_type, search_date, content) VALUES (?,6, unix_timestamp(now()),?);", [user.manager_id,search_text], function(err, row, fields) {
                        // 数据获取失败
       ...
       if(that.isEmpty(body.param_text)){
        ...
        db.query("select count(id) num from `db_people` a  where shopowner_id = ?  and is_prolocutor = '1' and is_shopowner = '0' AND ( a.nickname LIKE ? or a.realname LIKE ? or a.mobile like ? ) ", [body.shopowner_id,body.param_text,body.param_text,body.param_text] , function(err, row1, fields) {
                    if (err) {
                        db.release();// 释放资源
                        conn.end(); // 结束当前数据库链接
                    ...
                    var sql = "select manager_id,is_clerk,is_prolocutor,is_shopowner,headimgurl,id,mobile,nickname,realname,token from `db_people` a  where shopowner_id = ?  and is_prolocutor = '1' and is_shopowner = '0' AND ( a.nickname LIKE ? or a.realname LIKE ? or a.mobile like ? ) order by is_shopowner desc limit  " + _currentpage + "," + _pagesize;
