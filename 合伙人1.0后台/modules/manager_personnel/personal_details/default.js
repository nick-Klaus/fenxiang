...
exports.ready = (req, res, that) => {
    ...
    db.query("select a.username,count(c.id) order_num,a.realname,a.mobile,a.address,a.total_assets,a.amount, from ((`db_manager` a inner join `db_people` b on a.openid = b.openid) inner join `db_order` c on c.mid = a.id) inner join `db_activity_log` d on d.openid = b.openid where b.id = ? and c.status = 1 and c.pay_status = 3",body.id, function(err, row, fields) {
