...
exports.ready = (req, res, that) => {
    ...
    that.connect((err, db,conn) => {
            ...
            db.query("SELECT count(1) as num  from `db_order` a inner join `db_product` b on a.gid = b.id inner join `db_people` c on a.cid = c.id where a.commission>0 and a.status = 1 and a.pay_status=3  and ( a.mid = ?)  order by a.id desc ",[body.manager_id], function(err, row1, fields) {
                // 数据获取失败
                if (err) {
                    db.release(); // 释放资源
                    conn.end(); // 结束当前数据库链接
                ...
                var sql = "select a.*,b.name,c.nickname,c.mobile ,c.realname  from `db_order` a inner join `db_product` b on a.gid = b.id inner join `db_people` c on a.cid = c.id where a.commission>0 and  a.status = 1  and a.pay_status=3 and ( a.mid = ?)  order by a.id desc limit "+ _currentpage + "," + _pagesize;
