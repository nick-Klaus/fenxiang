    ...
    that.connect((err, db,conn) => {
        ...
        if(body.status == 1){
            ...
            db.query("SELECT count(1) as num  from `db_order` a inner join `db_product` b on a.gid = b.id inner join `db_people` c on a.cid = c.id where  a.status in (0,1) and  a.pay_status = 1 and ( a.sid = ?) AND ( a.title LIKE ? or a.pickup_information LIKE ?  or a.order_no LIKE ? )  and a.special_type=0  order by a.id desc ",[body.shopowner_id,body.param_text,body.param_text,body.param_text], function(err, row1, fields) {
              ...
              db.query("SELECT count(1) as num  from `db_order` a inner join `db_product` b on a.gid = b.id inner join `db_people` c on a.cid = c.id where  a.status in (0,1) and  a.pay_status = 2 and ( a.sid = ?) AND ( a.title LIKE ? or a.pickup_information LIKE ?  or a.order_no LIKE ? )  and a.special_type=0  order by a.id desc ",[body.shopowner_id,body.param_text,body.param_text,body.param_text], function(err, row1, fields) {
                // 数据获取失败
                if (err) {
                    db.release(); // 释放资源
                    conn.end(); // 结束当前数据库链接
                ...
                var sql = "select a.*,b.name,c.nickname,c.mobile,c.realname ,( select address from db_shop  where id = "+shopowner.shop_id+") shop_address  ,( select shop_name from db_shop  where id = "+shopowner.shop_id+") shop_name,( select phone from db_shop  where id = "+shopowner.shop_id+") shop_phone  from `db_order` a inner join `db_product` b on a.gid = b.id inner join `db_people` c on a.cid = c.id where  a.status in (0,1) and  a.pay_status = 2 and ( a.sid = ?) AND ( a.title LIKE ? or a.pickup_information LIKE ?  or a.order_no LIKE ? )  and a.special_type=0  order by a.id desc limit "+ _currentpage + "," + _pagesize;
                   ...
                   db.query("SELECT count(1) as num  from `db_order` a inner join `db_product` b on a.gid = b.id inner join `db_people` c on a.cid = c.id where  a.status in (0,1) and  a.pay_status = 4 and ( a.sid = ?) AND ( a.title LIKE ? or a.pickup_information LIKE ?  or a.order_no LIKE ? ) and a.special_type=0   order by a.id desc ",[body.shopowner_id,body.param_text,body.param_text,body.param_text], function(err, row1, fields) {
                // 数据获取失败
                if (err) {
                    db.release(); // 释放资源
                    conn.end(); // 结束当前数据库链接
                ...
                var sql = "select a.*,b.name,c.nickname,c.mobile,c.realname ,( select address from db_shop  where id = "+shopowner.shop_id+") shop_address  ,( select shop_name from db_shop  where id = "+shopowner.shop_id+") shop_name,( select phone from db_shop  where id = "+shopowner.shop_id+") shop_phone  from `db_order` a inner join `db_product` b on a.gid = b.id inner join `db_people` c on a.cid = c.id where  a.status in (0,1) and  a.pay_status = 4 and ( a.sid = ?) AND ( a.title LIKE ? or a.pickup_information LIKE ?  or a.order_no LIKE ? )  and a.special_type=0  order by a.id desc limit "+ _currentpage + "," + _pagesize;
                  ...
                  db.query("SELECT count(1) as num  from `db_order` a inner join `db_product` b on a.gid = b.id inner join `db_people` c on a.cid = c.id where  a.status in (0,1) and  a.pay_status = 3 and ( a.sid = ?) AND ( a.title LIKE ? or a.pickup_information LIKE ?  or a.order_no LIKE ? )  and a.special_type=0  order by a.id desc ",[body.shopowner_id,body.param_text,body.param_text,body.param_text], function(err, row1, fields) {
                // 数据获取失败
                if (err) {
                    db.release(); // 释放资源
                    conn.end(); // 结束当前数据库链接
                ...
                var sql = "select a.*,b.name,c.nickname,c.mobile,c.realname ,( select address from db_shop  where id = "+shopowner.shop_id+") shop_address  ,( select shop_name from db_shop  where id = "+shopowner.shop_id+") shop_name,( select phone from db_shop  where id = "+shopowner.shop_id+") shop_phone  from `db_order` a inner join `db_product` b on a.gid = b.id inner join `db_people` c on a.cid = c.id where  a.status in (0,1) and  a.pay_status = 3 and ( a.sid = ?) AND ( a.title LIKE ? or a.pickup_information LIKE ?  or a.order_no LIKE ? )  and a.special_type=0  order by a.id desc limit "+ _currentpage + "," + _pagesize;
                   ...
                   db.query("SELECT count(1) as num  from `db_order` a inner join `db_product` b on a.gid = b.id inner join `db_people` c on a.cid = c.id where  a.status in (0,1) and  a.pay_status = 5 and ( a.sid = ?) AND ( a.title LIKE ? or a.pickup_information LIKE ?  or a.order_no LIKE ? ) and a.special_type=0   order by a.id desc ",[body.shopowner_id,body.param_text,body.param_text,body.param_text], function(err, row1, fields) {
                // 数据获取失败
                if (err) {
                    db.release(); // 释放资源
                    conn.end(); // 结束当前数据库链接
                ...
                var _pagesize    = Number(body.pagesize); // 每页有多少数据
                var _currentpage = Number(body.currentpage); // 当前页
                var sql = "select a.*,b.name,c.nickname,c.mobile ,( select address from db_shop  where id = "+shopowner.shop_id+") shop_address ,( select shop_name from db_shop  where id = "+shopowner.shop_id+") shop_name,( select phone from db_shop  where id = "+shopowner.shop_id+") shop_phone   from `db_order` a inner join `db_product` b on a.gid = b.id inner join `db_people` c on a.cid = c.id where  a.status in (0,1) and  a.pay_status = 5 and ( a.sid = ?) AND ( a.title LIKE ? or a.pickup_information LIKE ?  or a.order_no LIKE ? ) and a.special_type=0   order by a.id desc limit "+ _currentpage + "," + _pagesize;
                    db.query(sql,[body.shopowner_id,body.param_text,body.param_text,body.param_text], function(err, row, fields) {
                    // 数据获取失败
                    if (err) {
                        db.release(); // 释放资源
