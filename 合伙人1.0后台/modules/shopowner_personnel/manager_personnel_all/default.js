...
exports.ready = (req, res, that) => {
    ...
    db.query("select count(a.id) num from (`db_people` a left join `db_shopowner` b on a.shopowner_id = b.id) left join `db_shop` c on b.shop_id = c.id where a.manager_id = ? AND is_manager='0' AND is_shopowner='0'  AND ( a.nickname LIKE ? or a.realname LIKE ? or a.mobile like ? ) ", [body.manager_id,body.param_text,body.param_text,body.param_text] , function(err3, row3, fields3) {
        if (err3) {
            db.release();// 释放资源
            conn.end(); // 结束当前数据库链接
        ...
        var sql =   "select a.*,c.shop_name from (`db_people` a left join `db_shopowner` b on a.shopowner_id = b.id) left join `db_shop` c on b.shop_id = c.id where a.manager_id = ? AND is_manager='0' AND is_shopowner='0' AND ( a.nickname LIKE ? or a.realname LIKE ? or a.mobile like ? ) limit  " + _currentpage + "," + _pagesize;    
