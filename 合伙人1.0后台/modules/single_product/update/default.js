...
exports.ready = (req, res, that) => {
    ...
    that.connect(( err , db , conn ) => {
        ...
        if( that.isEmpty(body.boss_id) || that.isEmpty(body.id) ){
            return res.errorEnd('条件不能为空', 200);
        }
        // 更新数据
        db.query( "UPDATE `db_single_product` SET ? where id=? and boss_id=?" , [ body ,body.id, body.boss_id ] , function( err, result , fields ){
