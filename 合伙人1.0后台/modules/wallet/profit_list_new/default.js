...
exports.ready = (req, res, that) => {
    ...
    that.connect((err, db,conn) => {
            ...
            db.query("SELECT count(id) as num FROM `all_in_out_money` WHERE  spokesman_id=? and ((in_out_type=1 and (type in (1,2,3,9,10,11) or (type=6 and finish_time is not null)) ) or (in_out_type=2 and type in (3)))  order by create_date desc  ", [body.id], function(err, row, fields) {
                ...
                var sql = "select  * FROM `all_in_out_money` WHERE  spokesman_id=? and ((in_out_type=1 and (type in (1,2,3,9,10,11) or (type=6 and finish_time is not null)) ) or (in_out_type=2 and type in (3)))   order by create_date desc limit  "+ _currentpage + "," + _pagesize;
