                    ...
                    db.query("SELECT * from  `db_people` where    id=? ",[body.fans_id], function(err, people, fields) {
                        ...
                        if(people[0].amount>=amount&&openid!=null&&openid!=""){
                            db.query(" UPDATE db_people SET openid_jhkj =? where id=?  ",[openid,body.fans_id], function(err, row, fields) {
                                ...
                                db.query( "select * from  db_manager_tixian  order by create_time desc limit 0,1 " , [] , function( err, db_manager_tixian_log , fields ){
                                    if( err ){
                                        db.rollback();
                                        db.release();// 释放资源
                                        conn.end(); // 结束当前数据库链接
                                    ...
                                    var order_no = "MT"+year+month+day+"1";
                                    if(db_manager_tixian_log.length>0){
                                        order_no = "MT"+year+month+day+(db_manager_tixian_log[0].id+1);
                                    }
